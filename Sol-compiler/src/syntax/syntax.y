...
%left TK_AND TK_OR
%nonassoc TK_EQUAL TK_NOTEQUAL '>' TK_GE '<' TK_LE TK_IN
%left '+' '-'
%left '*' '/'
%right UMINUS
/*for reference, see http://www.gnu.org/software/bison/manual/html_node/Contextual-Precedence.html#Contextual-Precedence*/
...
expr					:	expr TK_AND	expr {$$=initNode(NT_EXPR); $$->child=$1; $1->brother=initNode(T_AND); $1->brother->brother=$3;}
						|	expr TK_OR expr {$$=initNode(NT_EXPR); $$->child=$1; $1->brother=initNode(T_OR); $1->brother->brother=$3;}
						|	expr TK_EQUAL expr {$$=initNode(NT_EXPR); $$->child=$1; $1->brother=initNode(T_EQ); $1->brother->brother=$3;}
						|	expr TK_NOTEQUAL expr {$$=initNode(NT_EXPR); $$->child=$1; $1->brother=initNode(T_NE); $1->brother->brother=$3;}
						|	expr '>' expr {$$=initNode(NT_EXPR); $$->child=$1; $1->brother=initNode(T_GT); $1->brother->brother=$3;}
						|	expr TK_GE expr {$$=initNode(NT_EXPR); $$->child=$1; $1->brother=initNode(T_GE); $1->brother->brother=$3;}
						|	expr '<' expr {$$=initNode(NT_EXPR); $$->child=$1; $1->brother=initNode(T_LT); $1->brother->brother=$3;}
						|	expr TK_LE expr {$$=initNode(NT_EXPR); $$->child=$1; $1->brother=initNode(T_LE); $1->brother->brother=$3;}
						|	expr TK_IN expr {$$=initNode(NT_EXPR); $$->child=$1; $1->brother=initNode(T_IN); $1->brother->brother=$3;}
						|	expr '+' expr {$$=initNode(NT_EXPR); $$->child=$1; $1->brother=initNode(T_PLUS); $1->brother->brother=$3;}
						|	expr '-' expr {$$=initNode(NT_EXPR); $$->child=$1; $1->brother=initNode(T_MINUS); $1->brother->brother=$3;}
						|	expr '*' expr {$$=initNode(NT_EXPR); $$->child=$1; $1->brother=initNode(T_TIMES); $1->brother->brother=$3;}
						|	expr '/' expr {$$=initNode(NT_EXPR); $$->child=$1; $1->brother=initNode(T_DIVISION); $1->brother->brother=$3;}
						|	unary_op expr {$$=initNode(NT_EXPR); $$->child=$1; $1->brother=$2;}
						|	'(' expr ')' {$$=initNode(NT_EXPR); $$->child=$2;}
						|	left_hand_side {$$=initNode(NT_EXPR); $$->child=$1;}
						|	atomic_const {$$=initNode(NT_EXPR); $$->child=$1;}
						|	instance_construction {$$=initNode(NT_EXPR); $$->child=$1;}
						|	func_call {$$=initNode(NT_EXPR); $$->child=$1;}
						|	cond_expr {$$=initNode(NT_EXPR); $$->child=$1;}
						|	built_in_call {$$=initNode(NT_EXPR); $$->child=$1;}
						|	dynamic_input {$$=initNode(NT_EXPR); $$->child=$1;}
						;
						
unary_op				:	'-' %prec UMINUS {$$=initNode(NT_UNARY_OP); $$->child=initNode(T_UMINUS);}
